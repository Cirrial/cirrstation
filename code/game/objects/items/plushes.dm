...
/obj/item/toy/plush/proc/love(obj/item/toy/plush/Kisser, mob/living/user) //~<3
	...
	// Troutstation edit
	var/canbreed = !is_type_in_typecache(src, breeding_blacklist) && !is_type_in_typecache(Kisser, breeding_blacklist)
	...
	else if(Kisser.partner == src && canbreed && !plush_child) //the one advancing does not take ownership of the child and we have a one child policy in the toyshop
