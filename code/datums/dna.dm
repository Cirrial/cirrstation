...
/datum/dna/proc/generate_unique_features()
	...
	if(features["tail_anteater"]) // Troutstation edit
		L[DNA_ANTEATER_TAIL_BLOCK] = construct_block(SSaccessories.tails_list_anteater.Find(features["tail_anteater"]), length(SSaccessories.tails_list_anteater))
	if(features["anteater_markings"]) // Troutstation edit
		L[DNA_ANTEATER_MARKINGS_BLOCK] = construct_block(SSaccessories.anteater_markings_list.Find(features["anteater_markings"]), length(SSaccessories.anteater_markings_list))
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_ANTEATER_SNOUT_BLOCK) // Troutstation edit
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.anteater_snouts_list.Find(features["anteater_snout"]), length(SSaccessories.anteater_snouts_list)))
		if(DNA_ANTEATER_TAIL_BLOCK) // Troutstation edit
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.tails_list_anteater.Find(features["tail_anteater"]), length(SSaccessories.tails_list_anteater)))
		if(DNA_ANTEATER_MARKINGS_BLOCK) // Troutstation edit
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.anteater_markings_list.Find(features["anteater_markings"]), length(SSaccessories.anteater_markings_list)))
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["anteater_snout"]) // Troutstation edit
		dna.features["anteater_snout"] = SSaccessories.anteater_snouts_list[deconstruct_block(get_uni_feature_block(features, DNA_ANTEATER_SNOUT_BLOCK), length(SSaccessories.anteater_snouts_list))]
